...
namespace SIS.Service.Http
{
    public class HttpClientService : IDisposable
    {
       ...
       public HttpClientService(string serviceBaseAddress,string addressSuffix=null,string token = null)
       {
        ...
        public HttpClientService(string baseUrl,bool useCookie)
        {
        ...
        public HttpClientService(string serviceBaseAddress)
        {
        ...
        public T PUT<T, M>(string url, M model) where T : class where M : class
        {
            var requestMessage = new HttpRequestMessage();
            var objectContent = CreateJsonObjectContent(model);
            var responseMessage = _httpClient.PutAsync(url, objectContent).Result;
            var result = responseMessage.Content.ReadAsStringAsync().Result;
            if (responseMessage.StatusCode == HttpStatusCode.OK || responseMessage.StatusCode == HttpStatusCode.NoContent)
            {
                if (!string.IsNullOrEmpty(result))
